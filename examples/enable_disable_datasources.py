# swagger Examples - Enable disable datasources in bulk
#
# This script uses an input CSV (example: data_sources_test1.csv.csv)
# To Enable/Disable multiple vRealize Network Insight Data Sources. Modify data_sources_test1.csv.csv to contain your own data sources
# (vCenters, NSX, switches, firewalls)
# and run this script with the param --data_sources_csv to your CSV.

# Note: -
# DataSourceType in data_sources_test1.csv.csv is taken from swagger_client.models.data_source_type.py
...
def main(api_client, args):
    ...
    with open("{}".format(args.data_sources_csv), 'rb') as csvFile:
        ...
        for data_source in data_sources:
            ...
            datasource_fn = getattr(data_source_api, data_source_api_name[args.operation])
            try:
                ...
                if not entity_id:
                    ...
                    return
                datasource_fn(id=entity_id)
                logger.info("Successfully {}: {} : {}".format(args.operation, data_source_type, entity_id))
            except ApiException as e:
                print("Failed to {} data source type: {} : Error : {} ".format(args.operation, data_source_type, json.loads(e.body)))
                return_code = ERROR
...
def parse_arguments():
    ...
    parser.add_argument("--data_sources_csv", action="store",
                        default='data_sources_test1.csv', help="csv file with your own data sources")
    parser.add_argument("--operation", action="store",
                        default='enable', help="enable or disable")
