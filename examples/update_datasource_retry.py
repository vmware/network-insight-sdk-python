# Example: Update datasources in bulk (with retries)
#
# START Description
...
def update_snmp_config(entity_id, data_source_api, data_source):
    try:
        update_snmp_api_fn = getattr(data_source_api, get_api_function_name(
            data_source['DataSourceType'], 'update_snmp_config'))
        get_snmp_api_fn = getattr(data_source_api, get_api_function_name(
            data_source['DataSourceType'], 'get_snmp_config'))
        ...
        logger.error("Failed to get snmp config: Error : {} ".format(
            json.loads(e.body)))
...
def get_data_source_entity(get_datasource_fn, data_source_list, to_find):
    ...
    in_cache = next(iter(filter(
        lambda x: x.ip == to_find['IP'] or x.fqdn == to_find['fqdn'], datasource_cache)), None)
    if in_cache:
    ...
    for index, entity in enumerate(data_source_list.results):
        already_fetched = next(
            iter(filter(lambda x: x.entity_id == entity.entity_id, datasource_cache)), None)
        if already_fetched:
            # this entity is already in the cache and not the ds we're looking for
            continue

        # not in the cache, fetch it
        logger.debug("Fetching entity with ID: {}  ({} of {})".format(
            entity.entity_id, index + 1, len(data_source_list.results)))
...
def retry(fun, *args, **kwargs):
    ...
    for i in range(MAX_ATTEMPTS):
        try:
            ...
            logger.info("Failed to run <{}> (attempt {} of {}), {}".format(
                fun.__name__, i + 1, MAX_ATTEMPTS, json.loads(e.body)))
            last_error = ApiException(reason=json.loads(e.body))
            time.sleep(1)
        except Exception as e:
            logger.info("Failed to run <{}> (attempt {} of {}), {}".format(
                fun.__name__, i + 1, MAX_ATTEMPTS, traceback.format_exc()))
...
def main(api_client, args):
    ...
    total_lines = len(open(args.data_sources_csv).readlines()
    ...
    with open(args.data_sources_csv, 'rt') as csvFile:
        ...
        for csv_row in data_sources:
            ...
            logger.info("Adding: {} [{} of {}]".format(_get_label(
                csv_row), data_sources.line_num - 1, total_lines))
            # Get the Data source add api fn
            get_datasource_fn = getattr(
                data_source_api, get_api_function_name(data_source_type, 'get'))
            update_datasource_fn = getattr(
                data_source_api, get_api_function_name(data_source_type, 'update'))
            try:
                data_source_list = get_entities_by_type(
                    data_source_type, data_source_api)
                logger.info("Successfully got list of {} : Total Count : {}".format(
                    data_source_type, data_source_list.total_count))

                entity = get_data_source_entity(
                    get_datasource_fn, data_source_list, csv_row)
                if not entity:
                ...
                retry(update_datasource_fn, id=entity.entity_id,
                ...
                if csv_row['snmp_version']:
                    try:
                        logger.info("Attempting to update SNMP Config for {}".format(
                            _get_label(csv_row)))
                        retry(update_snmp_config, entity.entity_id,
                              data_source_api, csv_row)
                        logger.info("Successfully updated SNMP Config for {}".format(
                            _get_label(csv_row)))
                    except ApiException as e:
                        logger.error("Failed to update snmp config for {}: Error : {} ".format(
                ...
                if args.restart:
                    ...
                    disable_datasource_fn = getattr(
                        data_source_api, get_api_function_name(data_source_type, 'disable'))
                    disable_datasource_fn(entity.entity_id)
                    time.sleep(1)
                    logger.info("Starting datasource: {}".format(
                        _get_label(csv_row)))
                    enable_datasource_fn = getattr(
                        data_source_api, get_api_function_name(data_source_type, 'enable'))
...
if __name__ == '__main__':
    ...
    if failure_log:
        print("\n")
        print("-- API call failed for the following CSV line{}:".format(
            '' if len(failure_log) == 1 else 's'))
        for failure in failure_log:
            print(failure)
    if notfound_log:
        print("\n")
        print("-- Could not find datasource corresponding to the following CSV line{}:".format(
            '' if len(notfound_log) == 1 else 's'))
