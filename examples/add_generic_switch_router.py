# Example: Upload configs of generic router/switch
#
# START Description
...
# Run this script with the param --zip_file_path (path for ZIP file created using network-insight-sdk-generic-datasources) proxy_ip and ip or fqdn of physical device.
# END Description
#
...
def main(api_client, args):
    ...
    try:
        ...
        if not response:
            response = data_source_api.add_generic_switch(
                body=get_add_request_body(args, proxy_id))
            logger.info(
                "Successfully added: {} {} : Response : {}".format(data_source_type, args.device_ip_or_fqdn, response))
        data_source_api.file_upload(
            id=response.entity_id, file=args.zip_file_path)
...
def parse_arguments():
    ...
    parser.add_argument('--device_ip_or_fqdn', action='store',
                        help='switch or router IP or fqdn', required=True)
    parser.add_argument('--proxy_ip', action='store',
                        help='IP address of vRNI proxy', required=True)
    parser.add_argument("--zip_file_path", action="store",
                        help="zip file generated by uani")
