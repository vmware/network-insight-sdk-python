...
class MetricFilter:
    ...
    def pass_metric_filter(self, source=None, entry=None, message_group=None):
        ...
        if message_group == DatabusMessageGroup.METRICS.value:
            filter_table = f_manager.get_metric_filter_table()
        else:
        ...
        for m_id in match_ent["data"]["points"]:
            ...
            if key + ":" + match_ent["data"]["metric"] in filter_table:
                pass_through = self.check_match_unmatch(data_pass=True)
            else:
        ...
        if check_key + ":databus:databus" in f_manager.get_metric_filter_table():
            ...
            pass_through = self.check_match_unmatch(data_pass=False)
            return pass_through, entry
