...
class DatabusVmsQueue(DatabusQueue):
    ...
    def start_processing_data(self):
        ...
        def update_message(message, data):
            ...
            pass_through = non_metric_filter.validate_from_non_metric_filter(source=message["source"], entry=entry,
                                                                             message_group=DatabusMessageGroup.VMS)
            ...
            if pass_through:
                if entity_id in source_map:
                    if self.use_mongo:
                        ...
                        if push_db[0]:
                            self.logger.log(self.license_plate + "updated new vm in mongo -> {} ".format(entity_id))
                        else:
                            self.logger.log(
                                self.license_plate + "Error while pushing data to downstream: " + push_db[1])
                    else:
                        source_map.update({entity_id: message})
                        self.logger.log(self.license_plate + "updated vm -> {} ".format(entity_id))
                else:
                    # add new
                    if self.use_mongo:
                        ...
                        if push_db[0]:
                            self.logger.log(self.license_plate + "added new vm in mongo -> {} ".format(entity_id))
                        else:
